...
func CreateStock(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Stock with id %v created", stock.StockId)
}
...
func UpdateStock(w http.ResponseWriter, r *http.Request) {
	...
	msg := fmt.Sprintf("stock  updated successfully. Total rows/records affected %d", updateRows)
	res := response{
		ID:      int64(id),
		Message: msg,
	}
...
func DeleteStock(w http.ResponseWriter, r *http.Request) {
	...
	id, err := strconv.Atoi(params["id"])
	if err != nil {
		log.Fatalf("Unable to convert string to id %v", err)
	}

	deletedRows := deleteStock(int64(id))

	msg := fmt.Sprintf("stock deleted successfully. Total rows/records affected %d", deletedRows)
	res := response{
		ID:      int64(id),
		Message: msg,
	}
